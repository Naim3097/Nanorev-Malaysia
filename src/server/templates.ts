...
import type { Category, Product, Section, SectionType, Seo } from '@/types'

/** A template section carries un-filled copy, so its props are not yet a
 *  valid Section — {{tokens}} are unresolved and specs.rows may still be the
 *  '@productSpecs' sentinel. applyTemplate() turns these into real Sections. */
...
export function applyTemplate(
  ...
  const fill = (v: unknown): unknown => {
    if (typeof v === 'string') return v.replace(/\{\{(\w+)\}\}/g, (_, k: string) => tokens[k] ?? '')
  ...
  const sections = template.sections.map((s) => {
    const props = fill(JSON.parse(JSON.stringify(s.props))) as Record<string, unknown>
    ...
    // Every token is now filled and the sentinel resolved, so the props match
    // the section's declared shape — the one place that assertion is earned.
    return { type: s.type, props } as unknown as Section
  })
  return { sections, seo: fill({ ...template.seo }) as Seo }
}
