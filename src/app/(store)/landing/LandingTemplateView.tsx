...
import {
  BadgeCheck, ChevronRight, Droplets, Lock, ShieldCheck, ShoppingCart,
  Star, Store, ThermometerSun, Truck, Zap,
} from 'lucide-react'
...
const COPY = {
  headline: (p: Product) => `Protect your engine with ${p.name} ${p.grade}`,
  sub: (p: Product) =>
...
function packOptions(p: Product) {
...
export default function LandingTemplateView({ id }: { id?: string }) {
  const router = useRouter()
  const { productById, categoryById } = useCatalog()
          ...
          <div className="lp-benefit-grid">
            {BENEFITS.map(({ icon: BenefitIcon, title, text }) => (
              <div className="lp-benefit" key={title}>
                <div className="ic"><BenefitIcon size={22} /></div>
                <h4>{title}</h4>
                <p>{text}</p>
              </div>
            ))}
      ...
      <section className="section">
        <div className="wrap lp-spec-wrap">
          <div>
            ...
            <Link href={`/product/${product.id}`} className="btn btn-ghost">Full product page</Link>
          </div>
