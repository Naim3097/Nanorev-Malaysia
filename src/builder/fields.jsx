...
// every section type's form from its field declarations in schema.js —
// the equivalent of Nexova's per-element *Properties.tsx files.
...
export function Field({ field, value, onChange }) {
  ...
  if (kind === 'pairs') {
    const list = Array.isArray(value) ? value : []
    const setAt = (i, col, v) => onChange(list.map((row, j) => (j === i ? (col === 0 ? [v, row[1]] : [row[0], v]) : row)))
    return (
      <div className="bfield">
        <label>{label}</label>
        {list.map((row, i) => (
  ...
  if (kind === 'items') {
    const list = Array.isArray(value) ? value : []
    const blank = () => Object.fromEntries(field.itemFields.map((f) => [f.key, f.kind === 'number' ? 1 : f.kind === 'checkbox' ? false : f.kind === 'select' ? f.options[0] : '']))
    return (
      <div className="bfield">
        <label>{label}</label>
        {list.map((item, i) => (
...
export function SectionForm({ def, props, onChange }) {
  return (
    <>
      {def.fields.map((f) => (
