...
import type { IconName, SectionType } from '@/types'

/** One editable control in the properties panel. */
export type FieldDef =
  | { key: string; label: string; kind: 'text' | 'textarea' | 'number' | 'checkbox' }
  | { key: string; label: string; kind: 'select'; options: string[] }
  | { key: string; label: string; kind: 'strings' }
  | { key: string; label: string; kind: 'pairs' }
  | { key: string; label: string; kind: 'items'; itemFields: FieldDef[] }
...
// icon keys available to section configs (see ICONS in landing/sections.tsx)
export const ICON_OPTIONS: IconName[] = [
  'badge', 'drops', 'flame', 'gauge', 'heat', 'lock', 'noise', 'shield', 'star', 'truck', 'zap',
]

const icon = (key = 'icon', label = 'Ikon'): FieldDef => ({ key, label, kind: 'select', options: ICON_OPTIONS })
