...
import { Check, Package, Truck, Home as HomeIcon, MapPin, ClipboardCheck } from 'lucide-react'
...
export default function OrderSuccess() {
  ...
  useSeo({ title: 'Order Confirmed | NanoRev Malaysia', robots: 'noindex, nofollow' }, [])
    ...
    <div className="wrap page">
      <div className="success">
        ...
        <p style={{ color: 'var(--muted)' }}>
          Thank you, {details.name.split(' ')[0] || 'there'}. Your order <span className="ref">{ref}</span> is paid and being prepared.
        </p>

        <div className="track">
          {steps.map((s, i) => (
            <div className={`track-step ${i === 0 ? 'done' : ''}`} key={i}>
        ...
        <div className="receipt">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <span style={{ fontSize: '0.78rem', color: 'var(--muted)' }}>{receiptId}</span>
          </div>

          {items.map((i) => (
            <div className="receipt-row" key={i.id}>
              <span className="k">{i.qty} × {i.name} {i.grade} <small>({i.volume})</small></span>
          ...
          <div className="receipt-row">
            ...
            <span className="v" style={{ textAlign: 'right', maxWidth: 240 }}>
              {isDelivery ? `${details.address}${details.unit ? ', ' + details.unit : ''}, ${details.postcode}` : location?.name}
            </span>
          </div>
        ...
        <div className="note" style={{ display: 'flex', gap: 10, alignItems: 'center', textAlign: 'left', marginBottom: 24, padding: '14px 16px' }}>
          ...
          <div style={{ fontSize: '0.85rem' }}>
            <strong>{isDelivery ? 'Estimated dispatch' : 'Ready for pickup'}:</strong> {location?.eta || 'Same-day dispatch'} · A confirmation was sent to {details.email}.
          </div>
        </div>
